...
void ceph_rdma_open(struct ceph_connection *con)
{
	int ret;
...
void ceph_encode_headers(struct xio_iovec *encode, int msg_cnt, struct ceph_msg_header *hdr,
...
void ceph_rdma_send(struct ceph_connection *con)
{
	mutex_lock(&con->mutex);
	while (true) {
		u32 crc;
